...
function run() {
  ...
  const argv = yargs(hideBin(process.argv))
    ...
    .option("transparent-attributes", {
      describe:
        "Expose XML attributes as normal properties (no @_/underscore prefix) to the end-user",
      type: "boolean",
      default: false,
    })
    .option("template-file", {
      describe: "Path to a custom template file for class generation",
      type: "string",
      demandOption: false,
    })
    .option("template-tag-body", {
      describe:
        'Tag name in the template file for the constructor body (e.g., "tag-body")',
      type: "string",
      demandOption: false,
    })
    .option("template-tag-meta", {
      describe:
        'Tag name in the template file for the meta method (e.g., "tag-meta")',
      type: "string",
      demandOption: false,
    })
    .help()
  ...
  const generatedClasses = complexTypes
    .filter((typeDef) => typeDef["@_abstract"] !== "true") // <-- Only this line is added!
    .map((typeDef) => {
      const className = typeDef["@_name"];
      return buildClassCode(className, typeDef, argv, schemaObj);
    });
...
function buildClassCode(className, typeDef, argv) {
      ...
      if (elements) {
        elements.forEach((el) => {
          ...
          if (!type && el[`${XSD_PREFIX}simpleType`]) {
            ...
            if (!globalThis._inlineSimpleTypes)
              globalThis._inlineSimpleTypes = [];
          ...
          // For elements:
          const userPropName =
            argv["transparent-attributes"] && propName.startsWith("@_")
          ...
          properties.push({
            ...
            xsdType:
              el["@_type"] ||
              (el[`${XSD_PREFIX}simpleType`] ? "simpleType" : undefined),
    ...
    if (attributes) {
      ...
      attributes.forEach((attr) => {
        ...
        properties.push({
          ...
          xsdType:
            attr["@_type"] ||
            (attr[`${XSD_PREFIX}simpleType`] ? "simpleType" : undefined),
          isAttribute: true,
        });
      });
    }
  ...
  if (argv["template-file"]) {
    let template = fs.readFileSync(argv["template-file"], "utf-8");

    // Insert constructorBody
    if (argv["template-tag-body"]) {
      const tag = argv["template-tag-body"];
      const regex = new RegExp(`(<${tag}>)([\\s\\S]*?)(</${tag}>)`, "m");
      template = template.replace(regex, `$1\n${constructorBody}\n$3`);
    }

    // Insert metaMethod
    if (argv["template-tag-meta"]) {
      const tag = argv["template-tag-meta"];
      const regex = new RegExp(`(<${tag}>)([\\s\\S]*?)(</${tag}>)`, "m");
      template = template.replace(regex, `$1\n${metaMethod}\n$3`);
    }

    // Optionally, you can also replace ${className} and ${parentClass} in the template if you want:
...
function writeSingleFile(outputFile, generatedClasses, baseJsPath) {
  ...
  const baseCode = fs.readFileSync(baseJsPath, "utf-8").replace(/export /g, ""); // Remove exports for single-file bundling
